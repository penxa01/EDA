...
class ArbolBinario:
    ...
    def getPadreHno(self,hijo):
        if self.__raiz == None:
            print('Arbol Vacio')
        elif(not self.__izquierdo.vacio() and self.__izquierdo.getRaiz() == hijo):
            self.muestraHnoderecho()
            padre = self.__raiz
            print(padre)
            return padre
        elif(not self.__derecho.vacio() and self.__derecho.getRaiz() == hijo):
            self.muestraHnoizquierdo()
            padre = self.__raiz
            print(padre)
            return padre
        elif(self.__raiz < hijo):
            return self.__derecho.getPadreHno(hijo)
        elif(self.__raiz > hijo):
            return self.__izquierdo.getPadreHno(hijo)

    def muestraHnoderecho(self):
        if not self.__derecho.vacio():
            print("El hermano de {} es {}".format(self.__izquierdo.getRaiz(),self.__derecho.getRaiz()))
    
    def muestraHnoizquierdo(self):
        if not self.__izquierdo.vacio():
             print("El hermano de {} es {}".format(self.__derecho.getRaiz(),self.__izquierdo.getRaiz()))
...
if __name__ == "__main__":
    ...
    arbol.insertar(2)
    arbol.Inorden()
    hijo = int(input('Ingrese un valor del arbol\n'))
    print('El padre de {} es {}'.format(hijo,arbol.getPadreHno(hijo)))
